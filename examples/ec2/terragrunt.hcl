...
locals {
  ...
  namespace   = "brim"
  project     = "ec2-cicd" //replace(basename(get_repo_root()), "teraform-", "")
  environment = "ec2-cicd"
...
inputs = {
  ...
  ssm_document_target_key_values = "${module.context.id}"
}
